...
BEGIN
    IF EXISTS (SELECT 1 FROM USUARIO WHERE EMAIL_USUARIO = P_EMAIL_USUARIO) THEN
        ...
        SET MESSAGE_TEXT = 'Erro: O email já for criado, tente outro.';
    ELSE
        INSERT INTO USUARIO (NOME_USUARIO, EMAIL_USUARIO, SENHA_USUARIO,TIPO_CONTA, DATA_CADASTRO)
        VALUES (P_NOME_USUARIO, P_EMAIL_USUARIO, P_SENHA_USUARIO,P_TIPO_CONTA, NOW());
    END IF;
END //
DELIMITER ;
#TESTE
...
BEGIN
    SELECT ID_USUARIO, NOME_USUARIO, EMAIL_USUARIO, DATA_CADASTRO
    FROM USUARIO;
END //
DELIMITER ;
#TESTE
